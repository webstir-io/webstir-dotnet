...
internal sealed class PackagesReleaseCommand(
    ...
    : IPackagesSubcommand
{
    ...
    public IReadOnlyCollection<string> Aliases { get; } = new[] { "prepare" };
    ...
    private void LogBuildSummary(PackageBuildSummary summary)
    {
        ...
        if (summary.DryRun)
        {
            ...
            {
                _logger.LogInformation(
                    "[packages] Planned to {Action} package(s): {Packages}.",
                    summary.Publish ? "publish" : "build",
