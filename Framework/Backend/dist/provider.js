...
async function loadWorkspaceModuleManifest(workspaceRoot, buildRoot, entryPoints, diagnostics) {
...
function createManifest(buildRoot, artifacts, diagnostics, moduleManifest) {
    const entryPoints = [];
...
function shouldTypeCheck(mode, env) {
    if (mode === 'publish') {
        return true;
    }
...
async function discoverEntryPoints(sourceRoot) {
    const patterns = [
        'index.{ts,tsx,js,mjs}',
