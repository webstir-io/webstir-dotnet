...
public static class CssTransformer
{
    // CSS Modules Processing
    public static ProcessedCssModule ProcessModule(string content, string filePath)
    {
        if (!filePath.EndsWith(CssConstants.ModuleExtension, StringComparison.OrdinalIgnoreCase))
            return new ProcessedCssModule { Content = content, ClassMappings = [] };
