...
(() => {
  ...
  function report(e: ErrorEvent | PromiseRejectionEvent): void {
    try {
      const p = toPayload(e);
  ...
  const w = window as any;
  w.__WEBSTIR_ON_ERROR__ = w.__WEBSTIR_ON_ERROR__ || report;
  window.addEventListener('error', (e) => { try { w.__WEBSTIR_ON_ERROR__(e); } catch { /* ignore */ } });
  window.addEventListener('unhandledrejection', (e) => { try { w.__WEBSTIR_ON_ERROR__(e); } catch { /* ignore */ } });
})();
