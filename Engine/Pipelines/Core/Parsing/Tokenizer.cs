...
public class Tokenizer
{
    ...
    public Tokenizer(string input, string filePath, DiagnosticCollection? diagnostics = null)
    {
        _input = input;
        _filePath = filePath;
    ...
    private Token ReadMultiLineComment(int line, int col)
    {
        ...
        while (!IsAtEnd())
        {
            if (Peek() == '*' && PeekNext() == '/') { sb.Append(Advance()); sb.Append(Advance()); break; }
            sb.Append(Advance());
        }
    ...
    private Token ReadAtRule(int line, int col)
    {
        StringBuilder sb = new();
        sb.Append('@');
        while (!IsAtEnd() && IsLetter(Peek())) sb.Append(Advance());
    ...
    private Token ReadIdentifierOrKeyword(char first, int line, int col)
    {
        StringBuilder sb = new();
        sb.Append(first);
        while (!IsAtEnd() && (IsLetterOrDigit(Peek()) || Peek() == '_')) sb.Append(Advance());
